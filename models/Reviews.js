const reviews = [{
  "id": 1,
  "reviews": [ 
    "target cutting-edge networks",
    "dictumst morbi vestibulum velit id pretium iaculis diam erat fermentum justo nec condimentum",
    "urna pretium nisl ut volutpat sapien arcu sed augue aliquam erat volutpat in congue etiam" 
   ]
}, {
  "id": 2,
  "reviews": [ 
    "transition web-enabled initiatives",
    "at velit eu est congue elementum in hac habitasse platea dictumst"
   ]
}, {
  "id": 3,
  "reviews": [
    "matrix next-generation communities",
    "ultrices posuere cubilia curae duis faucibus accumsan odio curabitur convallis duis consequat dui nec nisi volutpat",
    "natoque penatibus et magnis dis parturient montes nascetur ridiculus mus etiam vel augue vestibulum rutrum rutrum neque aenean auctor"
  ]
}, {
  "id": 4,
  "reviews": "synergize collaborative e-commerce",
  "review2": "curabitur in libero ut massa volutpat convallis morbi odio odio elementum eu interdum eu tincidunt",
  "review3": "amet nulla quisque arcu libero rutrum ac lobortis vel dapibus at diam nam tristique"
}, {
  "id": 5,
  "reviews": [
     "syndicate dynamic bandwidth",
     "iterate web-enabled schemas",
     "iterate web-enabled schemas 22",
     "magna ac consequat metus sapien ut nunc vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae",
     "aliquam augue quam sollicitudin vitae consectetuer eget rutrum at lorem integer tincidunt ante vel ipsum"
  ]
}, {
  "id": 6,
  "reviews": [
    "optimize granular ROI",
    "mattis pulvinar nulla pede ullamcorper augue a suscipit nulla elit ac nulla sed vel enim sit",
    "nam dui proin leo odio porttitor id consequat in consequat ut nulla sed accumsan felis ut"
  ]
}, {
  "id": 7,
  "reviews": [
    "utilize innovative eyeballs",
    "vulputate vitae nisl aenean lectus pellentesque eget nunc donec quis condimentum curabitur",
    "pede libero quis orci nullam molestie nibh in lectus pellentesque at nulla"
  ]
}, {
  "id": 8,
  "reviews": [
    "target transparent ROI",
    "synthesize efficient platforms",
    "ut nunc vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae mauris viverra diam vitae",
    "luctus tincidunt nulla mollis molestie lorem quisque ut erat curabitur gravida nisi at nibh in hac habitasse platea dictumst aliquam"
  ]
}, {
  "id": 9,
  "reviews": []
}, {
  "id": 10,
  "reviews": [
    "expedite killer metrics",
    "tincidunt eu felis fusce posuere felis sed lacus morbi sem mauris",
    "mauris ullamcorper purus sit amet nulla quisque arcu libero rutrum ac lobortis vel dapibus at diam nam"
  ]
}];

export default reviews;
